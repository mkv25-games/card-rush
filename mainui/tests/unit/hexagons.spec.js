...
import { Hex, Point, DoubledCoord, OffsetCoord, Layout } from '../../src/utils/hex.js'
...
class Tests {
  ...
  static testHexRound () {
    const a = new Hex(0.0, 0.0, 0.0)
    const b = new Hex(1.0, -1.0, 0.0)
    const c = new Hex(0.0, -1.0, 1.0)
  ...
  static testLayout () {
    const h = new Hex(3, 4, -7)
    const flat = new Layout(Layout.flat, new Point(10.0, 15.0), new Point(35.0, 71.0))
    Tests.equalHex('layout', h, flat.pixelToHex(flat.hexToPixel(h)).round())
    const pointy = new Layout(Layout.pointy, new Point(10.0, 15.0), new Point(35.0, 71.0))
    Tests.equalHex('layout', h, pointy.pixelToHex(pointy.hexToPixel(h)).round())
  }

  static testOffsetRoundtrip () {
    const a = new Hex(3, 4, -7)
...
describe('Hexagons', () => {
  it('should not complain', () => {
    Tests.testAll()
    expect(complaints).to.deep.equal([])
  })
})
